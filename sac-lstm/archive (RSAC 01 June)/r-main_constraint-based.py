...
# config_file = 'sac-lstm/config_lstm.yaml' #when debugging, this works!!!
    ...
    def _(self):
        ...
        for e in range(E):
            ...
            for t in range(T-1):
                ...
                # -------Current state and action calculation---------------------
                if t==0:
                    ...
                    # if self.mat_reward[e, t-1] >= 90: # deterministic-like
                    if self.mat_satisfied_delay_constraint[e, t-1] == 1.0 and self.mat_satisfied_rate_constraint[e, t-1] == 1.0 and np.all(self.mat_fittingness_u_rate[e, :, t-1] >= 0.6):
                        print(style.RED + 'NICE! Episode: {}, Timestep: {}, Reward: {}'.format(e, t, self.mat_reward[e, t-1]))
                        #use the actions from the previous timestep
                        self.action = action_memory[t-1, :] # redundant
                        # just get the variables from the last timestep (We do not map again as they might be different than the last timestep's variables)
                        self.chi_compressed = self.mat_chi_compressed[e, :, t-1]
                        self.chi_num = self.mat_b_connected_episodic[e, :, t-1]
                        self.chi = self.mat_chi[e, :, :, t-1]
                        ###
                        self.rho_compressed = self.mat_rho_compressed[e, :, t-1]
                        #self.rho_num = X #Unneeded due to implementation
                        self.rho = self.mat_rho[e,:,:,:,t-1]
                        # self.unallocated_PRBs = X #Unneeded due to implementation
                        ###
                        self.p_compressed = self.mat_p_compressed[e, :, t-1]                        
                        #self.p_num = X #Unneeded due to implementation
                ...
                # -------Map actions to variables chi, rho, p ---------------------
                ...
                self.mat_rho[e,:,:,:,t] = self.rho # e* b*k*u * t
                ...
                for u in range(USER_NO):
                    self.mat_sum_power[e, u, t] = np.sum(self.p[:, :, u]) # summing the power allocated to all user PRBs
                ...
                #     if self.mat_satisfied_rate_constraint[e, t] < 1:
                #         print(style.CYAN + 'NOT MET! Episode: {}, Timestep: {}, Reward: {}'.format(e, t, self.mat_reward[e, t-1]))
...
plot_graph('Avg PRBs used per user for RSAC algorithm',
...
plot_graph("Comparison of Average E2E Delay (RSAC)",
...
plot_graph("Average Data Rate (RSAC)",
...
plot_graph("Average of total allocated power to each user (RSAC)",
...
print(style.UNDERLINE + "RSAC:Total time for {} timesteps ({} users) in {} Episdoes (aka iterations or epochs): {}".format(T, USER_NO, E, convert_seconds(np.sum(mat_episode_runtime))))
