...
class Specifications:
    def __init__(self, USER_NO, SLICE_NO, CONST_D_MAX, CONST_R_MIN, PACKET_SIZE):
        ...
        if CONST_D_MAX == -1:
            self.list_tolerable_delay = [10, 2, 20]# [20, 10, 100] # eMBB, URLLC, mMTC # for the control plane delay in DL (in ms)
        ...
        if CONST_R_MIN == -1:
            self.list_min_rate = [16, 3.8, 0.5] # [20, 1, 0.02] # eMBB #fromerly 100, URLLC, mMTC # user plane rate in Mbps
        ...
        if PACKET_SIZE == -1:
            self.packet_size = [0.008192, 0.00384, 0.000256] # 1024, 480, 32 bytes
             #[0.012, 0.000256, 0.0008] # in mbits # eMBB: 1500, URLLC: 32, mMTC: 100 bytes
        else:
