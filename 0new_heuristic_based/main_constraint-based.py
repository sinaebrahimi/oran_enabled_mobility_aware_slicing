...
np.random.seed(1322) # some random number
...
class _main_:
    def __init__(self, E, T):
        ...
        # -------------
        # Initialize matrices for Heuristic
    ...
    def _(self):
        #resetting the Heuristic agent here!  
        print(E,T)          
...
filename = f'O-RAN Heuristic, U={USER_NO}, PRB={PRB_NO}, BS={BS_NO}, E={E}, T={T}, VELOCITY={VELOCITY}, D_max={CONST_D_MAX}, R_min={CONST_R_MIN}.npz'
...
# %%%%%Delay%%%%%%
# Calculate mean delay over users for Heuristic
mean_delay_sac = np.mean(np.mean(monte_mat_delay_tot, axis=1), axis=0)
# Plot the comparison graph
plot_graph("Comparison of Average E2E Delay (Heuristic)",
...
plot_graph("Average Data Rate (Heuristic)",
...
plot_graph("Average of total allocated power to each user (Heuristic)",
